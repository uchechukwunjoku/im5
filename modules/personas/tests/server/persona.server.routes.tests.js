 ...
 * Globals
 */
var app, agent, credentials, user, persona;
...
describe('Persona CRUD tests', function() {
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/api/personas')
					...
					.end(function(personaSaveErr, personaSaveRes) {
						...
						agent.put('/api/personas/' + personaSaveRes.body._id)
							.send(persona)
							.expect(200)
	...
	it('should be able to get a single Persona if not signed in', function(done) {
		...
		// Save the Persona
		personaObj.save(function() {
			request(app).get('/api/personas/' + personaObj._id)
				.end(function(req, res) {
					// Set assertion
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/api/personas')
					...
					.end(function(personaSaveErr, personaSaveRes) {
						...
						agent.delete('/api/personas/' + personaSaveRes.body._id)
							.send(persona)
							.expect(200)
	...
	it('should not be able to delete Persona instance if not signed in', function(done) {
		...
		personaObj.save(function() {
			...
			request(app).delete('/api/personas/' + personaObj._id)
			.expect(403)
			.end(function(personaDeleteErr, personaDeleteRes) {
				// Set message assertion
				(personaDeleteRes.body.message).should.match('User is not authorized');
